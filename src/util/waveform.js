...
class Waveform {
    constructor(data = [], xOffset = 0, xScale = 1) {
        ...
        this._monotoneousAscending = false;
        this.dataInUseType = "";
        this.dataInUse = { x: [], y: [] };
        this._dataAggregated = {};
        this._dataAggregationDirection = "";
    ...
    append(x, y) {
        ...
        if (this._monotoneous) {
            if (y > this._data[this._data.length - 1] &&
                this.getMonotoneousAscending() === false) {
                this._monotoneous = false;
            }
            else if (y < this._data[this._data.length - 1] &&
    ...
    _setData(dataY) {
        ...
        let i = 1, monoDir = dataY[1] > dataY[0], minY = dataY[0], maxY = dataY[0];
        if (isNaN(minY)) {
    ...
    checkMinMaxErrorBars() {
        let minY = this.minY, maxY = this.maxY, i = 0, l = this.getLength();
    ...
    computeXMinMax() {
        ...
        if (this._xdata) {
            ...
            const b1 = this.xOffset + this.xScale * this.getLength(), b2 = this.xOffset;
    ...
    */
    ...
    checkMonotonicity() {
        let i = 1, data = this.getData(false);
    ...
    resampleForDisplay(options) {
        ...
        let inverting = false, dataY = this.getDataY(), data = {
            x: [],
            y: []
        }, dataMinMax = [], resampleSum, resampleMin, resampleMax, resampleNum, resample_x_start, resample_x_px_start, x_px, doing_mean = false, firstPointIndex = 0, xval;
        ...
        if ((this._xdata && !this._xdata.getMonotoneousAscending()) ||
        ...
        for (; inverting ? i > 0 : i < l; inverting ? i-- : i++) {
            ...
            if (Math.abs(x_px - resample_x_px_start) > options.resampleToPx ||
                i == l ||
                i == 0 ||
                isNaN(dataY[i])) {
    ...
    interpolate(x) {
        ...
        if (this._xdata) {
            ...
            return (((x - xData[xIndex]) / (xData[xIndex + 1] - xData[xIndex])) *
            ...
            return ((xIndex - xIndexF) * (yData[xIndexF + 1] - yData[xIndexF]) +
                yData[xIndexF]);
        }
    }
    interpolateIndex_X(index) {
        ...
        if (this._xdata) {
            ...
            return ((index - indexStart) * (xData[indexStart + 1] - xData[indexStart]) +
                xData[indexStart]);
        }
    }
    ...
    _waveArithmetic(wave, operation) {
        let yDataThis = this.getDataY(), i = 0;
        const l = this.getLength();
        this.requireXMonotonicity();
    ...
    subrangeX(fromX, toX) {
        if (!this._xdata) {
            ...
            let fromP = this.getIndexFromX(fromX), toP = this.getIndexFromX(toX);
            return new Waveform()
    ...
    findLocalMinMax(xRef, xWithin, type) {
        let index = this.getIndexFromX(xRef), indexPlus = this.getIndexFromX(xRef + xWithin), indexMinus = this.getIndexFromX(xRef - xWithin);
...
function getIndexInterpolate(value, valueBefore, valueAfter, indexBefore, indexAfter) {
    return (((value - valueBefore) / (valueAfter - valueBefore)) *
...
function euclidianSearch(targetX, targetY, haystackX, haystackY, scaleX = 1, scaleY = 1) {
    let distance = Number.MAX_VALUE, distance_i;
    let index = -1;
    if (targetX !== undefined && targetY == undefined) {
        ...
        for (var i = 0, l = haystackX.length; i < l; i++) {
            distance_i =
                Math.pow(((targetX - haystackX[i]) * scaleX), 2) +
                    Math.pow(((targetY - haystackY[i]) * scaleY), 2);
            if (distance_i < distance) {
                index = i;
                distance = distance_i;
            }
...
function binarySearch(target, haystack, reverse = haystack[haystack.length - 1] < haystack[0], fineCheck = true) {
    let seedA = 0, length = haystack.length, seedB = length - 1, seedInt, i = 0, nanDirection = 1;
    if ((!reverse && (haystack[0] > target || haystack[seedB] < target)) ||
