...
export var TickPosition;
(function (TickPosition) {
    TickPosition["OUTSIDE"] = "outside";
    TickPosition["INSIDE"] = "inside";
    TickPosition["CENTERED"] = "centered";
...
class Axis extends EventEmitter {
    ...
    adapt() {
        ...
        var axis = this.options.adaptTo.axis, current = this.options.adaptTo.thisValue, foreign = this.options.adaptTo.foreignValue;
    ...
    handleMouseWheel(delta, e, baseline) {
        ...
        this._doZoomVal(((this.getCurrentMax() - baseline) * (1 + delta)) + baseline, ((this.getCurrentMin() - baseline) * (1 + delta)) + baseline, false);
        this.graph.draw();
     ...
     * @return {Axis} The current axis
     */
    setMinMaxToFitSeries(noNotify = false) {
    ...
    */
    ...
    _draw() {
      ...
      */
        ...
        if (!this.options.hideTicks) {
            console.log(this._primaryTickIncrement, this, this.linkedToAxis, this.options.logScale);
    ...
    drawTicks(primary, secondary) {
        var unitPerTick = primary, min = this.getCurrentMin(), max = this.getCurrentMax(), secondaryIncr, incrTick, subIncrTick, loop = 0;
        if (secondary) {
    ...
    drawLogTicks() {
        var min = this.getCurrentMin(), max = this.getCurrentMax();
    ...
    drawLinkedToAxisTicksWrapper(widthPx, valrange) {
        var opts = this.linkedToAxis, px = 0, val, t, l, delta2;
    ...
    modifyUnit(value, mode) {
        ...
        switch (mode) {
            case 'time': // val must be in seconds => transform in hours / days / months
                var max = this.getModifiedValue(this.getMaxValue()), units = [
                    [60, 'min'],
                    [3600, 'h'],
                    [3600 * 24, 'd']
                ];
