...
class Serie extends EventEmitter {
  ...
  constructor(graph, name, options) {
    ...
    this.graph = graph;
    this.name = name;
    this.options = Object.assign({}, Serie.default(), this.constructor.default(), options); //if( new.target.default ) {
...
class SerieLine extends Serie {
  ...
  setOptions(options) {
    this.options = extend(true, {}, SerieLine.prototype.defaults, options || {}); // Unselected style
  ...
  _getMarkerIndexFromEvent(e) {
    var px = this.graph._getXY(e); //  return this.searchIndexByPxXY( ( px.x ), ( px.y ) );
  ...
  searchIndexByPxXY(x, y) {
    ...
    for (var k = 0, m = this.waveform.getLength(); k < m; k += 1) {
      ...
      dist = Math.pow(this.getX(p_x) - x, 2) + Math.pow(this.getY(p_y) - y, 2); //console.log(x, y, dist, this.data[i][k], this.data[i][k + 1]);
  ...
  getMax(start, end) {
    var start2 = Math.min(start, end),
        ...
        max = -Infinity,
        initJ,
        maxJ; //      console.log( start2, end2, v1, v2 );
  ...
  _recalculateMarkerPoints(selectionType, families) {
    var markerPoints = []; // Overwriting any other undefined families
    ...
    for (var i = 0, k = families.length; i < k; i++) {
      ...
      for (var j = 0, l = families[i].points.length; j < l; j++) {
        if (families[i].points[j] == 'all') {
          ...
          markerPoints.push([families[i].points[j], families[i].points[j], i]); //markerPoints.push( [ family[ i ].points[ j ] + 1, null ] );
        } else {
    ...
    this.markerFamilies[selectionType || 'unselected'] = families; // Let's sort if by the first index.
...
class SerieDensityMap extends Serie {
  ...
  calculateDensityWeighted(fromX, deltaX, numX, fromY, deltaY, numY) {
    ...
    for (i = 0; i < l; i++) {
      ...
      compY = 1 - (exactY - indexY); //console.log( exactY, indexY );
  ...
  calculateDensityAdvanced() {
    ...
    for (var i in this.densityMapCalculation) {
      ...
      if (this.densityMapCalculation[i].pxPerBin) {
        ...
        results[i].num = Math.ceil(widthValues[i] / this.densityMapCalculation[i].pxPerBin); //console.log( from, from - axisGetter[ i ].call( this ).getRelVal( ( results[i ].num * this.densityMapCalculation[ i ].pxPerBin - dimension ) / 2 ), ( results[i ].num * this.densityMapCalculation[ i ].pxPerBin - dimension ) / 2 );
...
class PluginSerieLineDifference extends Plugin {
  ...
  draw() {
    ...
    for (; i1 < this.serie1.data.length; i1++) {
      for (; j1 < this.serie1.data[i1].length; j1 += 2) {
        ...
        if (!s2) {
          ...
          if (s2) {
            i2 = s2.dataIndex;
            j2 = s2.xBeforeIndex * 2; // TODO: Add here first points
    ...
    this.negativePolyline.setPointsPx(d).redraw(); //pathsBottom.map( function( map ) { makePaths( map, self.options.negativeStyle ); } );
...
var SplitAxis = function (mixin) {
  ...
  var cl = class SplitAxis extends mixin {
    ...
    _splitSpread() {
      let splits = [],
          total = 0,
          currentSplit = 0; //console.log( this._splitVal );
